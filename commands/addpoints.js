...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option.setName('target').setDescription('Member to give points to').setRequired(true))
    .addIntegerOption(option =>
      option.setName('amount').setDescription('Number of points to add').setMinValue(1).setRequired(true))
  ...
  async execute(interaction) {
    ...
    await interaction.reply(
      `➕ Added ${amount} **${type}** points to ${user.tag}. Total: ${record.amount}`
    );
